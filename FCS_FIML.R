...
analyze.Data <- function(simDataMissing, numCat)
{
  ...
  for (i in 1:imputationMI)
  {
    ...
    mydata <- complete(miceImputation,
                       action = i,
                       inc = FALSE)
  ...
  # rename the columns
  names(containimpute) <- c("o1", "o2", "o3", "c1", "c2", "c3",
                            "o1.SE", "o2.SE", "o3.SE", "c1.SE", "c2.SE", "c3.SE", 
                            "Convergence")
  ...
  {
    pooled <- pool.scalar(Q = containimpute[[myvec[r]]], 
                          # Q: A vector of univariate estimates of m repeated complete data analyses.
                          U = containimpute[[paste0(myvec[r], ".SE")]]) 
